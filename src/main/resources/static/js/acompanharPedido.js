window.onload = async () => {
	...
	document.querySelector(".previsaoDeEntrega p").innerHTML = pedido.previsaoDeEntrega;
...
async function getPedido(pedidoId) {
	const pedidos = await fetch(`http://localhost:8080/pedidos/${pedidoId}`).then(response => response.json());
